...
def es_tema_educacion_basica(texto):
    ...
    similitud_jaccard = jaccard_similarity(palabras_clave_educacion_basica, palabras_texto)

    umbral_similitud = 0.2
    return similitud_jaccard > umbral_similitud
...
def generate_response():
    ...
    if not es_tema_educacion_basica(input_text):
        return jsonify({"response": "El texto ingresado no está relacionado con la educación básica."})
    prompt = f"hola, buenos días, buenas tardes, buenas noches, saludos, qué, cómo, donde, cuándo, calcula, cuanto es, cuanto mide, de donde viene, por favor, cuantos grados, cuantos tipos, por qué, quien, de qué forma, de qué manera, dame, ejercicios, concepto, definición, cuál, cuales, figurar, desarrolar, cuando nació, ser muy amigable en el contexto de la educación básica en: {input_text}"
