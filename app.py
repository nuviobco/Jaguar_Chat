...
def generate_response():
    ...
    if not request.json:
        return jsonify({"error": "Se esperaba una solicitud en formato JSON"}), 400

    if "input_text" not in request.json:
        return jsonify({"error": "No se ha proporcionado el valor 'input_text'"}), 400
